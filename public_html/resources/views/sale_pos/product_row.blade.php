...
<tr class="product_row" data-row_index="{{$row_count}}" @if(!empty($so_line)) data-so_id="{{$so_line->transaction_id}}" @endif>
	...
	@if(!empty($is_direct_sell))
		...
		<td @if(!$edit_discount) class="hide" @endif>
			...
			<!--{!! Form::select("products[$row_count][line_discount_type]", [ 'percentage' => __('lang_v1.percentage'),'fixed' => __('lang_v1.fixed')], $discount_type , ['class' => 'form-control row_discount_type']); !!}-->
			{!! Form::select("products[$row_count][line_discount_type]", [ 'percentage' => __('lang_v1.percentage')], $discount_type , ['class' => 'form-control row_discount_type']); !!}
			@if(!empty($discount))
